...
def estimador(df: pd.DataFrame, atributo: str, categoria: int, n_projecao: int) -> tuple:
        ...
        return np.min(valores_projetados), np.max(valores_projetados), np.mean(valores_projetados), np.std(valores_projetados)
...
class BayPredict:
    ...
    #######################################
        ...
        for i in range(n_months):
            ...
            upper_lim = upper_lim + [val_max]
            lower_lim = lower_lim + [val_min]
            mean = mean + [val_mean]
    ...
    def show_predictions(self, plot=False, savepath='default.svg'):
        ...
        for percentage in np.array(mean_lim):
            mean.append(lower[-1] + lower[-1]*percentage)
