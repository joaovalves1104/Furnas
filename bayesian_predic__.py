...
class BayPredict:
    ...
    def get_norm_cat_params(self, df:pd.DataFrame,  n_bins_dict='', quartis=False):
        ...
        if normalization_type == norm_type.CLUSTER:
            ...
            return {'base_values': df.pipe(self.simple_normalization, return_params=True), 'boundaries': df.apply(self.simple_normalization).apply(self.k_means_disc, args=(n_bins_dict, True))}
    ...
    def normalize_and_categorize_data(self, df:pd.DataFrame, n_bins_dict='', quartis=False)->pd.DataFrame:
        ...
        if normalization_type == norm_type.CLUSTER:
            ...
            output_dataframe = df.apply(self.simple_normalization).apply(self.k_means_disc, args=(n_bins_dict))
    ...
    def create_observed_inference_file(self, include_var:list,savepath='modelo_arquivo_4.json'):
        n_clusters = self.N_CAT
        ...
        for element in include_var:
            exclude.remove(element)

        columns = self.DESEASONED_DF.columns
        columns_0 = [column + ':0' for column in columns]

        inference_dict = {}
        base_normalizada = self.normalize_and_categorize_data(self.DESEASONED_DF)
    ...
    def generate_dynamic_topology(self, time_conections=''):
        ...
        # DBN_top_0_1 = [((node, 0), (node, 1)) for node in node_set if node != self.TARGET_VRIABLE]
    ...
    def get_predictions(self,
        ...
        df = self.DESEASONED_DF[lambda x: x.index.year<=self.STARTING_YEAR].copy()
    ...
    #######################################
        ...
        #     upper_lim = upper_lim + [parametros['base_values'][variable] * parametros['boundaries'][variable][predictions[i]][1] + (upper_lim[i-1] if i!= 0 else 0)]
        #     lower_lim = lower_lim + [parametros['base_values'][variable] * parametros['boundaries'][variable][predictions[i]][0] + (lower_lim [i-1] if i!= 0 else 0)]
        ...
        # for i in range(n_months):
        #     val_min, val_max, val_mean, desvio_padrao = estimador(val_cat_df, variable, categoria=predictions[i], n_projecao= 100)

        #     upper_lim = upper_lim + [val_max + (upper_lim[i-1] if i!= 0 else 0)]
        #     lower_lim = lower_lim + [val_min + (lower_lim [i-1] if i!= 0 else 0)]
        #     mean.append(val_mean + (mean[i-1] if i!= 0 else 0))

        #     #upper_lim.append(mean[-1] + val_max)
        #     #lower_lim.append(mean[-1] - val_min)

        #     variacao_mensal_categorias.update({'MES_' + str(i): {'categoria':predictions[i], 'probabilidade': probability[i]}})
       
         ###################
        
        ################### SUAVISACAO POR PERCENTIL
        val_cat_df = self.get_value_cat_df().loc[:, [self.TARGET_VRIABLE, self.TARGET_VRIABLE + '_CAT']]
        alpha = 0.95
        
        for i in range(n_months):

            array = val_cat_df.query(f'{self.TARGET_VRIABLE + "_CAT"} == {predictions[i]}')[self.TARGET_VRIABLE]
            lower = np.percentile(array, (1 - alpha) / 2 * 100)
            upper = np.percentile(array, (alpha + (1 - alpha) / 2) * 100)

            upper_lim = upper_lim + [upper + (upper_lim[i-1] if i!= 0 else 0)]
            lower_lim = lower_lim + [lower + (lower_lim [i-1] if i!= 0 else 0)]
    ...
    def show_predictions(self, g_df=None, plot=False, savepath='default.svg'):
        ...
        x_ = df[lambda x: x.index.year<=int(self.STARTING_YEAR)].index
        last_month = x_[-1]
        ...
        if type(g_df) != type(None):
            prev_ob = prev_ob.loc[:g_df.index[-1],:]
        ...
        if plot:
            ...
            df_previsoes['upper_lim'].plot(linestyle='--', alpha=0.7, linewidth=0.7, ax=ax)
            df_previsoes['lower_lim'].plot(linestyle='--', alpha=0.7, linewidth=0.7, ax=ax)
            df_previsoes['mean'].plot(ax=ax)
            df[self.TARGET_VRIABLE].plot(ax=ax)

            if type(g_df) != type(None):
                plt.xlim([g_df.index[1], g_df.index[-1]])
    
            plt.legend(['Limite Superior','Limite Inferior', 'Previsão', 'Valor Observado'], loc='lower left')
            #plt.xlabel('Ano')
            #plt.ylabel('Consumo Faturado (MWh)')
            #plt.savefig(savepath)

            mape = mean_absolute_percentage_error(prev_ob[self.TARGET_VRIABLE], prev_ob['mean'])
            rmse = mean_squared_error(prev_ob[self.TARGET_VRIABLE], prev_ob['mean'])
            mae = mean_absolute_error(prev_ob[self.TARGET_VRIABLE], prev_ob['mean'])

            plt.text(0.02, 0.9, f'RMSE = {rmse**(1/2):.2f}\nMAE = {mae:.2f}\nMAPE = {mape:.4f}',
    ...
    def get_cat_limits(self):
        limits_dict = self.get_norm_cat_params(self.DESEASONED_DF)
        f_limits_dict = {}
